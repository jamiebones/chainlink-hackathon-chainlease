...
var configSchema = exports_external.object({
  evms: exports_external.object({
    chainSelectorName: exports_external.string(),
    leaseAgreementAddress: exports_external.string(),
    gasLimit: exports_external.string()
  }),
  creditCheckApi: exports_external.object({
    endpoint: exports_external.string(),
    apiKey: exports_external.string()
  }),
  backendApi: exports_external.object({
    endpoint: exports_external.string(),
    apiKey: exports_external.string().optional()
  }).optional()
});
var leaseCreatedEventArgsSchema = exports_external.object({
  leaseId: exports_external.bigint(),
  propertyId: exports_external.bigint(),
  tenant: exports_external.string().regex(/^0x[a-fA-F0-9]{40}$/, "Invalid Ethereum address"),
  monthlyRent: exports_external.bigint().positive()
});
function fetchCreditCheck(sendRequester, config, dataToSend) {
  const queryParams = new URLSearchParams({
    tenantAddress: dataToSend.tenantAddress,
    leaseId: dataToSend.leaseId.toString(),
    propertyId: dataToSend.propertyId.toString()
  });
  const req = {
...
function submitCreditCheckResult(runtime2, leaseId, passed, verificationId) {
  runtime2.log(`Submitting credit check result for lease ${leaseId.toString()}: ${passed}`);
  runtime2.log(`Verification ID: ${verificationId}`);
  const evmConfig = runtime2.config.evms;
  ...
  const reportData = encodeAbiParameters(parseAbiParameters("uint256 leaseId, bool passed, string verificationId"), [leaseId, passed, verificationId]);
  runtime2.log(`Writing report to consumer contract at ${evmConfig.leaseAgreementAddress}`);
  try {
    ...
    runtime2.log("Report generated, submitting to chain...");
    ...
    if (writeReportResult.txStatus === TxStatus.SUCCESS) {
      ...
      runtime2.log(`Credit check result submitted successfully: ${txHash}`);
      return txHash;
    } else if (writeReportResult.txStatus === TxStatus.REVERTED) {
      runtime2.log(`Transaction reverted: ${writeReportResult.errorMessage || "Unknown error"}`);
...
var onLogTrigger = (runtime2, log) => {
  ...
  const result = httpClient.sendRequest(runtime2, fetchCreditCheck, consensusIdenticalAggregation())(runtime2.config, leaseData).result();
  if (!result || result.statusCode !== 200) {
    runtime2.log(`Credit check API failed for tenant ${tenant} on lease ${leaseId.toString()}`);
    submitCreditCheckResult(runtime2, leaseId, false, "api-error");
    return `Credit check API failed for lease ${leaseId.toString()}`;
  }
  submitCreditCheckResult(runtime2, BigInt(result.leaseId), result.passed, result.verificationId);
  runtime2.log(`Credit check completed: ${result.passed ? "PASSED" : "FAILED"} (Score: ${result.creditScore})`);
